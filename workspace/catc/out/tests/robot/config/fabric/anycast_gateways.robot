...
Check Anycast Gateways Configuration in Data Model
    ...
    Log   Found 2 fabric site(s) with anycast gateways in data model
...
Verify Anycast Gateways for Fabric Site Global/Poland/Warsaw
    # Check if this site should be skipped based on MANAGED_SITES
    ...
    ${fabric_entry}=   Set Variable   ${fabric_data}[0]
    ${fabric_id}=   Get From Dictionary   ${fabric_entry}   id
    Log To Console   Fabric ID for Global/Poland/Warsaw: ${fabric_id}
    # Validate anycast gateways exist for this fabric
    ${gateway_data}=   Get Value From Json   ${r.json()}   $.response[?(@.fabricId=='${fabric_id}')]
    Run Keyword If   not ${gateway_data}   Fail   No anycast gateways found for fabricId ${fabric_id} in Anycast Gateways API response.
    Log To Console   Gateway data for fabric ${fabric_id}: ${gateway_data}
...
Validate CampusVN2-IPPool
    # Check if the parent fabric site is managed (skip if parent was skipped)
    ${should_skip}=   Should Skip Site Validation   Global/Poland/Warsaw
    Pass Execution If   ${should_skip}   Gateway CampusVN2-IPPool in fabric site Global/Poland/Warsaw not managed by this deployment state - skipping validation
    # Validate individual gateway CampusVN2-IPPool
    ${gateway_match}=   Get Value From Json   ${gateway_data}   $[?(@.ipPoolName=='CampusVN2-IPPool')]
    Run Keyword If   not ${gateway_match}   Fail   Gateway CampusVN2-IPPool not found in Anycast Gateways API response.
    ${g}=   Set Variable   ${gateway_match}[0]
    Log To Console   Extracted Gateway Entry: ${g}
    # Validate basic gateway attributes
    Run Keyword And Continue On Failure   Should Be Equal Value Json String   ${g}   $.ipPoolName   CampusVN2-IPPool
    Run Keyword And Continue On Failure   Should Be Equal Value Json String   ${g}   $.virtualNetworkName   Campus
    Run Keyword And Continue On Failure   Should Be Equal Value Json String   ${g}   $.trafficType   DATA
    # Validate VLAN configuration
    Log To Console   Skipping VLAN name validation as auto_generate_vlan_name is enabled for CampusVN2-IPPool
    # Check if this is a special pool type (EXTENDED_NODE or FABRIC_AP)
    ${pool_type}=   Run Keyword And Return Status   Dictionary Should Contain Key   ${g}   poolType
    # Validate boolean gateway attributes only for regular pools (not EXTENDED_NODE or FABRIC_AP)
    IF    not ${pool_type}
...
Validate GuestVN2-IPPool
    # Check if the parent fabric site is managed (skip if parent was skipped)
    ${should_skip}=   Should Skip Site Validation   Global/Poland/Warsaw
    Pass Execution If   ${should_skip}   Gateway GuestVN2-IPPool in fabric site Global/Poland/Warsaw not managed by this deployment state - skipping validation
    # Validate individual gateway GuestVN2-IPPool
    ${gateway_match}=   Get Value From Json   ${gateway_data}   $[?(@.ipPoolName=='GuestVN2-IPPool')]
    Run Keyword If   not ${gateway_match}   Fail   Gateway GuestVN2-IPPool not found in Anycast Gateways API response.
    ${g}=   Set Variable   ${gateway_match}[0]
    Log To Console   Extracted Gateway Entry: ${g}
    # Validate basic gateway attributes
    Run Keyword And Continue On Failure   Should Be Equal Value Json String   ${g}   $.ipPoolName   GuestVN2-IPPool
    Run Keyword And Continue On Failure   Should Be Equal Value Json String   ${g}   $.virtualNetworkName   Guest
    Run Keyword And Continue On Failure   Should Be Equal Value Json String   ${g}   $.trafficType   DATA
    # Validate VLAN configuration
    Log To Console   Skipping VLAN name validation as auto_generate_vlan_name is enabled for GuestVN2-IPPool
    # Check if this is a special pool type (EXTENDED_NODE or FABRIC_AP)
    ${pool_type}=   Run Keyword And Return Status   Dictionary Should Contain Key   ${g}   poolType
    # Validate boolean gateway attributes only for regular pools (not EXTENDED_NODE or FABRIC_AP)
    IF    not ${pool_type}
...
Validate BYOD2-IPPool
    # Check if the parent fabric site is managed (skip if parent was skipped)
    ${should_skip}=   Should Skip Site Validation   Global/Poland/Warsaw
    Pass Execution If   ${should_skip}   Gateway BYOD2-IPPool in fabric site Global/Poland/Warsaw not managed by this deployment state - skipping validation
    # Validate individual gateway BYOD2-IPPool
    ${gateway_match}=   Get Value From Json   ${gateway_data}   $[?(@.ipPoolName=='BYOD2-IPPool')]
    Run Keyword If   not ${gateway_match}   Fail   Gateway BYOD2-IPPool not found in Anycast Gateways API response.
    ${g}=   Set Variable   ${gateway_match}[0]
    Log To Console   Extracted Gateway Entry: ${g}
    # Validate basic gateway attributes
    Run Keyword And Continue On Failure   Should Be Equal Value Json String   ${g}   $.ipPoolName   BYOD2-IPPool
    Run Keyword And Continue On Failure   Should Be Equal Value Json String   ${g}   $.virtualNetworkName   BYOD
    Run Keyword And Continue On Failure   Should Be Equal Value Json String   ${g}   $.trafficType   DATA
    # Validate VLAN configuration
    Log To Console   Skipping VLAN name validation as auto_generate_vlan_name is enabled for BYOD2-IPPool
    # Check if this is a special pool type (EXTENDED_NODE or FABRIC_AP)
    ${pool_type}=   Run Keyword And Return Status   Dictionary Should Contain Key   ${g}   poolType
    # Validate boolean gateway attributes only for regular pools (not EXTENDED_NODE or FABRIC_AP)
    IF    not ${pool_type}
...
Validate PrintersVN2-IPPool
    # Check if the parent fabric site is managed (skip if parent was skipped)
    ${should_skip}=   Should Skip Site Validation   Global/Poland/Warsaw
    Pass Execution If   ${should_skip}   Gateway PrintersVN2-IPPool in fabric site Global/Poland/Warsaw not managed by this deployment state - skipping validation
    # Validate individual gateway PrintersVN2-IPPool
    ${gateway_match}=   Get Value From Json   ${gateway_data}   $[?(@.ipPoolName=='PrintersVN2-IPPool')]
    Run Keyword If   not ${gateway_match}   Fail   Gateway PrintersVN2-IPPool not found in Anycast Gateways API response.
    ${g}=   Set Variable   ${gateway_match}[0]
    Log To Console   Extracted Gateway Entry: ${g}
    # Validate basic gateway attributes
    Run Keyword And Continue On Failure   Should Be Equal Value Json String   ${g}   $.ipPoolName   PrintersVN2-IPPool
    Run Keyword And Continue On Failure   Should Be Equal Value Json String   ${g}   $.virtualNetworkName   Printers
    Run Keyword And Continue On Failure   Should Be Equal Value Json String   ${g}   $.trafficType   DATA
    # Validate VLAN configuration
    Log To Console   Skipping VLAN name validation as auto_generate_vlan_name is enabled for PrintersVN2-IPPool
    # Check if this is a special pool type (EXTENDED_NODE or FABRIC_AP)
    ${pool_type}=   Run Keyword And Return Status   Dictionary Should Contain Key   ${g}   poolType
    # Validate boolean gateway attributes only for regular pools (not EXTENDED_NODE or FABRIC_AP)
    IF    not ${pool_type}
