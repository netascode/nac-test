...
"""
[PoC] Consolidated IOS-XE Device Verifications using Dynamic Loop Marking
--------------------------------------------------------------------------
This PoC demonstrates running multiple verification types in a SINGLE subprocess
using PyATS native aetest.loop.mark() pattern to eliminate subprocess overhead.

Performance Goal:
  - Current: 11 test files → 11 run() calls → 11 subprocesses → 99s overhead
  - Target: 11 verification types → 1 run() call → 1 subprocess → 9s overhead
...
# PoC Verification Configs (1 type for clean timing - mock only supports this command)
VERIFICATION_CONFIGS = {
    "sdwan_control": {
        "title": "Verify All SD-WAN Control Connections Are Up",
        "description": "Validates that all SD-WAN control connections to vSmart and vManage controllers are in 'up' state",
...
class CommonSetup(aetest.CommonSetup):
    ...
    def mark_verification_loops(self):
        ...
        This eliminates 10 subprocess spawns (11 types - 1 subprocess = 10 saved).
        """
...
class DeviceVerification(IOSXETestBase):
    ...
    def test_device_verification(self, verification_type: str, steps):
        ...
        Args:
            verification_type: Key from VERIFICATION_CONFIGS (e.g., "sdwan_control")
    ...
    def get_items_to_verify(self):
        ...
        For PoC, returns single item per type. Production could return
    ...
    async def _verify_sdwan_config_sync(self, semaphore, client, context):
        ...
        This is a simplified implementation for PoC demonstration.
        Production version would parse 'show sdwan system status' output.
        """
        async with semaphore:
            try:
                command = self.TEST_CONFIG["api_endpoint"]
