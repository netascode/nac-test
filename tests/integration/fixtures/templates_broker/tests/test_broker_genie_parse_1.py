...
"""
[NRFU]: Verify OSPF MPLS TE Links via Genie-Driven Execution (Broker Cache Test 1)
-----------------------------------------------------------------------------------
...
This is the FIRST of two identical test files. When both run on the same device,
the broker should show:
- File 1: 2 command cache misses per device (primary + supplementary)
- File 2: 2 command cache hits per device (both commands cached)
"""
...
class BrokerGenieParseTest1(IOSXETestBase):
    """
