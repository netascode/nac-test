...
from tests.conftest import inject_context, resolve_and_inject_context
...
class TestBaseTestControllerDetection:
    ...
    def test_base_test_detects_controller_on_setup(
        ...
        """Test that NACTestBase uses resolved controller context during setup."""
    ...
    def test_base_test_fails_setup_on_detection_error(
        self, setup_test_data_file_env: Path
    ) -> None:
        """Test that NACTestBase fails setup when controller context is missing.

        Relies on the global autouse ``clean_controller_env`` fixture in
        ``tests/conftest.py`` ensuring ``ENV_CONTROLLER_CONTEXT`` is unset.
        """
    ...
    def test_base_test_no_longer_uses_controller_type_env_var(
        ...
        """Test that NACTestBase uses resolved context and ignores CONTROLLER_TYPE."""
