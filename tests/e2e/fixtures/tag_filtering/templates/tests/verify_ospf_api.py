...
TITLE = "Verify OSPF API Test"
DESCRIPTION = "Simple OSPF verification test via API for tag filtering e2e tests."
...
class VerifyOspfApi(APICTestBase):
    groups = ["ospf", "api-only"]

    TEST_CONFIG = {
        "resource_type": "OSPF API Test",
    ...
    async def verify_item(self, semaphore, client, context):
        async with semaphore:
            ...
            context["display_context"] = "OSPF API Test"

            if response.status_code == 200:
                return self.format_verification_result(
                    status=ResultStatus.PASSED,
                    context=context,
                    reason="OSPF API check passed.",
