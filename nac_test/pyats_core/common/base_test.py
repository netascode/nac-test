...
class NACTestBase(aetest.Testcase):
            ...
            try:
                ...
                if not reporter:
                    if attempt < max_retries - 1:
                        ...
                        self.logger.error("PyATS reporter unavailable after %d attempts", max_retries)
                        break
                ...
                if messages_sent > 0:
                    pyats_success = True
                    self.logger.debug("Sent %d/%d messages to PyATS reporter", messages_sent, len(messages))
                    break
                ...
                if attempt < max_retries - 1:
                    ...
                    self.logger.error("PyATS reporter connection failed after %d attempts: %s", max_retries, e)
                ...
                self.logger.error("Unexpected error sending to PyATS reporter: %s", e, exc_info=True)
                break
    ...
    def _send_single_message_to_pyats(self, reporter: Any, message: Dict[str, Any], 
                                     metadata: Optional[Any] = None) -> bool:
    ...
    def _handle_batching_error(self, error: Exception, messages: List[Dict[str, Any]]) -> None:
        ...
        try:
            ...
            if hasattr(self, 'output_dir') and self.output_dir:
                try:
                    ...
                    self.logger.debug("Cannot create emergency directory in output dir: %s", e)
            ...
            self.logger.error(
                "Recovery command: cat %s | python -m json.tool",
                dump_file
            )
            ...
            for attempt in range(MAX_RETRIES):
                try:
                    ...
                    if not isinstance(e, (httpx.HTTPError, httpx.RemoteProtocolError)):
                        ...
                        if not any(term in error_msg for term in 
                                 ['timeout', 'connection', 'disconnected', 'protocol', 'http', 
                                  'socket', 'network', 'refused', 'reset', 'broken']):
