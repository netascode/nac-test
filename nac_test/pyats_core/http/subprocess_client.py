...
"""Fork-safe HTTP client for macOS compatibility.

On macOS, after PyATS uses fork() to create task subprocesses, httpx.AsyncClient
crashes silently due to OpenSSL threading primitives that are not fork-safe.

This module provides SubprocessHttpClient which makes HTTP requests via subprocess
using urllib (which is fork-safe). The client implements the same async interface
as httpx.AsyncClient, allowing it to be used as a drop-in replacement.

Implementation Details:
    On macOS, after fork():
    - subprocess.run() crashes due to pipe creation issues
    - asyncio.to_thread() uses ThreadPoolExecutor which breaks
    - asyncio.create_subprocess_exec() child watcher breaks
...
Performance Note:
    This client is slower than httpx.AsyncClient due to subprocess overhead.
    It should only be used when fork-safety is required (i.e., on macOS after
    fork()). On Linux and other platforms, httpx.AsyncClient should be used
