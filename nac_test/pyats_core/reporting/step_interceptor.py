...
class StepInterceptor:
    ...
    def is_batching_enabled(self) -> bool:
        ...
        env_enabled = os.environ.get('NAC_TEST_BATCHING_REPORTER', 'false').lower() == 'true'
        
        # Also check our internal enabled flag (for fallback scenarios)
        return env_enabled and self.enabled
    
    def wrap_step_enter(self, original_enter):
    ...
    def wrap_step_exit(self, original_exit):
        ...
        """
        @wraps(original_exit)
        def wrapped_exit(step_self, exc_type, exc_value, traceback):
            ...
            try:
                ...
                if hasattr(step_self, 'parent') and hasattr(step_self.parent, 'reporter'):
                ...
                try:
                    ...
                    if saved_parent_reporter is not None:
                        ...
                        if logger.isEnabledFor(logging.DEBUG):
                            logger.debug(
                                "Restored parent reporter for step: %s",
                                str(step_self)
                            )
                ...
                if logger.isEnabledFor(logging.DEBUG):
                    logger.debug(
                        ...
                        step_info['name'], step_info['result'], context_path
                    )
    ...
    def _build_context_path(self, step) -> str:
    ...
    def disable(self) -> None:
        ...
        if self.intercepted_count > 10 and self.error_count / self.intercepted_count > 0.1:
    ...
    def get_stats(self) -> dict:
        ...
        return {
            ...
            'error_rate': (
                ...
                if self.intercepted_count > 0 else 0
            )
        }
