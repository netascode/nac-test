...
class OverflowDetector:
            ...
            if self.ema_rate > self.BURST_THRESHOLD:
                if not self.burst_detected:
                    ...
                    logger.info(
                        ...
                        self.ema_rate, self.BURST_THRESHOLD
                    )
                return True
            else:
                if self.burst_detected:
                    duration = current_time - self.burst_start_time
                    logger.info(
                        "Burst ended after %.1f seconds. Rate normalized to %.1f msg/sec",
                        duration, self.ema_rate
                    )
                    self.burst_detected = False
                    self.burst_start_time = None
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'burst_duration': (
                time.time() - self.burst_start_time 
                if self.burst_start_time else 0
            )
        }
...
class OverflowQueue:
    ...
    def __init__(
        ...
        self.memory_limit_mb = memory_limit_mb or int(
            os.environ.get('NAC_TEST_MEMORY_LIMIT_MB', str(self.DEFAULT_MEMORY_LIMIT_MB))
        )
        self.memory_limit_bytes = self.memory_limit_mb * 1024 * 1024
        
        # Queue and memory tracking
        self.queue: queue.Queue = queue.Queue(maxsize=self.max_size)
        ...
        logger.debug(
            ...
            self.max_size, self.memory_limit_mb
        )
    ...
    def enqueue(self, message: Any, metadata: MessageMetadata) -> bool:
        ...
        with self.lock:
            ...
            try:
                ...
                logger.warning(
                    "Queue full (%d messages). Overflowing to disk.",
                    self.max_size
                )
                self._overflow_to_disk(message, metadata)
                return False
    ...
    def dequeue_batch(self, max_count: int = 100, timeout: float = 0.1) -> List[Tuple[Any, MessageMetadata]]:
    ...
    def _overflow_to_disk(self, message: Any, metadata: MessageMetadata) -> None:
        ...
        filename = self.overflow_dir / f"overflow_{timestamp}_{self.overflow_file_count:06d}.json"
...
class WorkerThread:
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'time_since_drain': (
                time.time() - self.last_drain_time 
                if self.last_drain_time else 0
            )
        }
...
class BatchAccumulator:
    ...
    def __init__(
        ...
        self.debug_mode = debug_mode or os.environ.get('NAC_TEST_DEBUG', '').lower() == 'true'
        ...
        self.overflow_callback = None  # Called when burst detected
        
        logger.debug(
            "BatchAccumulator initialized with batch_size=%d, timeout=%.2fs, debug=%s",
            self.default_batch_size, self.flush_timeout, self.debug_mode
        )
...
class BatchingReporter:
    ...
    def __init__(
        ...
        self.debug_mode = debug_mode if debug_mode is not None else (
            os.environ.get('NAC_TEST_DEBUG', '').lower() == 'true'
        )
        ...
        logger.info(
            ...
            self.batch_size, self.flush_timeout, self.debug_mode
        )
    ...
    def _handle_batch_flush(self, batch_info: Dict[str, Any]) -> None:
        ...
        if self.overflow_mode_active and self.overflow_queue:
            ...
            logger.info(
                ...
                messages_queued, messages_overflowed
            )
        else:
            ...
            if self.send_callback:
                try:
                    ...
                    if success:
                        ...
                        logger.warning(
                            "Failed to send batch: %d messages",
                            batch_info['count']
                        )
        ...
        if self.send_callback:
            try:
                ...
                if success:
                    ...
                    logger.warning("Failed to send drained batch of %d messages", len(batch))
            except Exception as e:
            ...
            logger.debug(
                "No send callback - dropped batch: %d messages from queue",
                len(batch)
            )
    ...
    def flush(self) -> int:
        ...
        if self.overflow_queue:
            ...
            if queue_size > 0:
                logger.warning(
                    "Overflow queue contains %d messages pending processing",
                    queue_size
                )
        ...
        if self.overflow_queue:
            ...
            if count > 0:
                logger.error(
                    ...
                    count, self.overflow_queue.overflow_dir
                )
