...
class TerminalColors:
    ...
    @classmethod
    def format_env_var_error(cls, missing_vars: list[str], controller_type: str = "ACI") -> str:
        ...
        """
        lines = []
        lines.append(cls.header(f"ERROR: Missing {controller_type} environment variable(s)"))
        
        for var in missing_vars:
            lines.append(f"  {cls.warning('•')} {cls.warning(var)}")
        
        lines.append("")
        lines.append(cls.info(f"Please set the required {controller_type} environment variables before running tests."))
