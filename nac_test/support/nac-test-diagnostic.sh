...
# Use mktemp for secure temp directory creation (prevents race conditions)
DIAG_DIR=$(mktemp -d "${TMPDIR:-/tmp}/nac-test-diagnostics-XXXXXX")
if [ ! -d "$DIAG_DIR" ]; then
    echo -e "${RED}ERROR: Failed to create diagnostic directory${NC}"
    exit 1
fi
...
mask_credentials_in_file() {
    ...
    # Use mktemp for secure temp file creation (prevents race conditions)
    local temp_file=$(mktemp "${file}.masking.XXXXXX")
    cp "$file" "$temp_file"
    ...
    # SSH Private Keys (PEM format) - mask entire key block
    sed -i.bak '/-----BEGIN[[:space:]]\{1,\}.*PRIVATE KEY-----/,/-----END[[:space:]]\{1,\}.*PRIVATE KEY-----/c\
-----BEGIN PRIVATE KEY-----\
***PRIVATE_KEY_MASKED***\
-----END PRIVATE KEY-----' "$temp_file" 2>/dev/null

    # OAuth client secrets and refresh tokens
    sed -i.bak 's/[Cc]lient[_-]*[Ss]ecret[[:space:]]*[:=][[:space:]]*[^[:space:]"]*/client_secret=***MASKED***/g' "$temp_file" 2>/dev/null
    sed -i.bak 's/"[Cc]lient[_-]*[Ss]ecret"[[:space:]]*:[[:space:]]*"[^"]*"/"client_secret": "***MASKED***"/g' "$temp_file" 2>/dev/null
    sed -i.bak 's/[Rr]efresh[_-]*[Tt]oken[[:space:]]*[:=][[:space:]]*[^[:space:]"]*/refresh_token=***MASKED***/g' "$temp_file" 2>/dev/null
    sed -i.bak 's/"[Rr]efresh[_-]*[Tt]oken"[[:space:]]*:[[:space:]]*"[^"]*"/"refresh_token": "***MASKED***"/g' "$temp_file" 2>/dev/null

    # AWS-style access keys (AKIA...)
    sed -i.bak 's/AKIA[0-9A-Z]\{16\}/***AWS_ACCESS_KEY_MASKED***/g' "$temp_file" 2>/dev/null

    # SNMP community strings
    sed -i.bak 's/[Cc]ommunity[[:space:]]*[:=][[:space:]]*[^[:space:]"]*/community=***MASKED***/g' "$temp_file" 2>/dev/null
    sed -i.bak 's/"[Cc]ommunity"[[:space:]]*:[[:space:]]*"[^"]*"/"community": "***MASKED***"/g' "$temp_file" 2>/dev/null
...
collect() {
...

    {
        ...
        bash -c "$cmd" 2>&1 || echo "COMMAND FAILED WITH EXIT CODE: $?"
    } > "$output_file"
...
[ -n \"\$APIC_PASSWORD\" ] && echo 'APIC_PASSWORD=*** SET ***' || echo 'APIC_PASSWORD=(not set)'
[ -n \"\$ACI_USERNAME\" ] && echo 'ACI_USERNAME=*** SET ***' || echo 'ACI_USERNAME=(not set)'
[ -n \"\$ACI_PASSWORD\" ] && echo 'ACI_PASSWORD=*** SET ***' || echo 'ACI_PASSWORD=(not set)'
[ -n \"\$SDWAN_USERNAME\" ] && echo 'SDWAN_USERNAME=*** SET ***' || echo 'SDWAN_USERNAME=(not set)'
[ -n \"\$SDWAN_PASSWORD\" ] && echo 'SDWAN_PASSWORD=*** SET ***' || echo 'SDWAN_PASSWORD=(not set)'
[ -n \"\$CC_USERNAME\" ] && echo 'CC_USERNAME=*** SET ***' || echo 'CC_USERNAME=(not set)'
[ -n \"\$CC_PASSWORD\" ] && echo 'CC_PASSWORD=*** SET ***' || echo 'CC_PASSWORD=(not set)'
[ -n \"\$ISE_USERNAME\" ] && echo 'ISE_USERNAME=*** SET ***' || echo 'ISE_USERNAME=(not set)'
[ -n \"\$ISE_PASSWORD\" ] && echo 'ISE_PASSWORD=*** SET ***' || echo 'ISE_PASSWORD=(not set)'
echo ''

echo '--- Device Credentials (D2D) ---'
[ -n \"\$IOSXE_USERNAME\" ] && echo 'IOSXE_USERNAME=*** SET ***' || echo 'IOSXE_USERNAME=(not set)'
[ -n \"\$IOSXE_PASSWORD\" ] && echo 'IOSXE_PASSWORD=*** SET ***' || echo 'IOSXE_PASSWORD=(not set)'
echo ''

echo '--- API Keys ---'
[ -n \"\$MERAKI_API_KEY\" ] && echo 'MERAKI_API_KEY=*** SET ***' || echo 'MERAKI_API_KEY=(not set)'
echo ''
...
[ -n \"\$NAC_TEST_PYATS_SSH_CONCURRENCY\" ] && echo \"NAC_TEST_PYATS_SSH_CONCURRENCY=\$NAC_TEST_PYATS_SSH_CONCURRENCY\" || echo 'NAC_TEST_PYATS_SSH_CONCURRENCY=(not set)'
[ -n \"\$NAC_TEST_PYATS_OUTPUT_BUFFER_LIMIT\" ] && echo \"NAC_TEST_PYATS_OUTPUT_BUFFER_LIMIT=\$NAC_TEST_PYATS_OUTPUT_BUFFER_LIMIT\" || echo 'NAC_TEST_PYATS_OUTPUT_BUFFER_LIMIT=(not set)'
[ -n \"\$NAC_TEST_PYATS_KEEP_REPORT_DATA\" ] && echo \"NAC_TEST_PYATS_KEEP_REPORT_DATA=\$NAC_TEST_PYATS_KEEP_REPORT_DATA\" || echo 'NAC_TEST_PYATS_KEEP_REPORT_DATA=(not set)'
[ -n \"\$NAC_TEST_PYATS_OVERFLOW_DIR\" ] && echo \"NAC_TEST_PYATS_OVERFLOW_DIR=\$NAC_TEST_PYATS_OVERFLOW_DIR\" || echo 'NAC_TEST_PYATS_OVERFLOW_DIR=(not set)'
echo ''

echo '--- NAC Test Internal Tuning (undocumented) ---'
[ -n \"\$NAC_TEST_NO_TESTLEVELSPLIT\" ] && echo \"NAC_TEST_NO_TESTLEVELSPLIT=\$NAC_TEST_NO_TESTLEVELSPLIT\" || echo 'NAC_TEST_NO_TESTLEVELSPLIT=(not set)'
[ -n \"\$NAC_TEST_PYATS_SENTINEL_TIMEOUT\" ] && echo \"NAC_TEST_PYATS_SENTINEL_TIMEOUT=\$NAC_TEST_PYATS_SENTINEL_TIMEOUT\" || echo 'NAC_TEST_PYATS_SENTINEL_TIMEOUT=(not set)'
[ -n \"\$NAC_TEST_PYATS_PIPE_DRAIN_DELAY\" ] && echo \"NAC_TEST_PYATS_PIPE_DRAIN_DELAY=\$NAC_TEST_PYATS_PIPE_DRAIN_DELAY\" || echo 'NAC_TEST_PYATS_PIPE_DRAIN_DELAY=(not set)'
[ -n \"\$NAC_TEST_PYATS_PIPE_DRAIN_TIMEOUT\" ] && echo \"NAC_TEST_PYATS_PIPE_DRAIN_TIMEOUT=\$NAC_TEST_PYATS_PIPE_DRAIN_TIMEOUT\" || echo 'NAC_TEST_PYATS_PIPE_DRAIN_TIMEOUT=(not set)'
[ -n \"\$NAC_TEST_PYATS_QUEUE_SIZE\" ] && echo \"NAC_TEST_PYATS_QUEUE_SIZE=\$NAC_TEST_PYATS_QUEUE_SIZE\" || echo 'NAC_TEST_PYATS_QUEUE_SIZE=(not set)'
[ -n \"\$NAC_TEST_PYATS_MEMORY_LIMIT_MB\" ] && echo \"NAC_TEST_PYATS_MEMORY_LIMIT_MB=\$NAC_TEST_PYATS_MEMORY_LIMIT_MB\" || echo 'NAC_TEST_PYATS_MEMORY_LIMIT_MB=(not set)'
[ -n \"\$NAC_TEST_PYATS_BATCH_SIZE\" ] && echo \"NAC_TEST_PYATS_BATCH_SIZE=\$NAC_TEST_PYATS_BATCH_SIZE\" || echo 'NAC_TEST_PYATS_BATCH_SIZE=(not set)'
[ -n \"\$NAC_TEST_PYATS_BATCH_TIMEOUT\" ] && echo \"NAC_TEST_PYATS_BATCH_TIMEOUT=\$NAC_TEST_PYATS_BATCH_TIMEOUT\" || echo 'NAC_TEST_PYATS_BATCH_TIMEOUT=(not set)'
[ -n \"\$NAC_TEST_BATCHING_REPORTER\" ] && echo \"NAC_TEST_BATCHING_REPORTER=\$NAC_TEST_BATCHING_REPORTER\" || echo 'NAC_TEST_BATCHING_REPORTER=(not set)'
"
...

{
    ...
    echo ""
    # Execute command directly without eval to prevent command injection
    # The command is passed as a single string and executed by bash in a subshell
    bash -c "$NAC_TEST_COMMAND" 2>&1
    NAC_TEST_EXIT=$?
    echo ""
